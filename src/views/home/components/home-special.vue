<template>
  <HomePannel title="最新专题" ref="target">
    ...
    <div class="special-list" ref="homeSpecial">
      <div class="special-item" v-for="item in list" :key="item.id">
        ...
        <div class="foot">
          <span class="like"><i class="iconfont icon-jushoucang"></i>{{item.collectNum}}</span>
          <span class="view"><i class="iconfont icon-yanjing"></i>{{item.viewNum}}</span>
          <span class="reply"><i class="iconfont icon-wodefankui"></i>{{item.replyNum}}</span>
        </div>
      </div>
    </div>
  </HomePannel>
