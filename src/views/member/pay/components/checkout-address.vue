  ...
  <div class="checkout-address">
    ...
    <xtx-dialog :title="'切换收货地址'" v-model:visiable="visiable">
      <div @click="selectedAddress=item" :class="{active:selectedAddress&&selectedAddress.id===item.id}"
        class="text item" v-for="item in userAddressList" :key="item.id">
        <ul>
      ...
      <template #footer>
        <XtxButton @click="visiable=false" type="gray" style="margin-right:20px">取消</XtxButton>
...
<script>
  ...
  export default {
    ...
    setup(props, {
      ...
      const defaultAddress = props.userAddressList.find(item => item.isDefault === 0);
      if (defaultAddress) {
