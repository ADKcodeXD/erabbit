<template>
  ...
  <div class="sub-filter" v-if="filterData && !filterLoading ">
    <div class="item">
      ...
      <div class="body">
        <a @click="changeBrand(item.id)" :class="{active:item.id===filterData.brands.selectBrand}" href="javascript:;" v-for="item in filterData.brands" :key="item.id">{{item.name}}</a>
      </div>
    </div>
    <!-- 第二部分区域 -->
    <div class="item" v-for="item in filterData.saleProperties" :key="item.id">
      ...
      <div class="body">
        <a @click="changeProp(item.properties,properties.id)" :class="{active:properties.id===item.properties.selectedAttr}" href="javascript:;" v-for="properties in item.properties" :key="properties.id">{{properties.name}}</a>
      </div>
    </div>
  </div>
  <!-- 骨架屏 -->
